...
void C_Application::init(HINSTANCE hinst) {
	...
	G_Graphics->init(d_window, d_installer->isFullScreen());
...
bool C_Application::msgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	switch(msg) {
	case WM_KEYDOWN:
		if(G_Console->isActive() || wParam == VK_OEM_3) {
	...
	case WM_LBUTTONDOWN:
		if(d_pMouseTarget) {
			d_pMouseTarget->onButtonDown(I_MouseListener::E_LEFT, LOWORD(lParam), HIWORD(lParam));
		}
		break;
	case WM_LBUTTONUP:
		if(d_pMouseTarget) {
			d_pMouseTarget->onButtonUp(I_MouseListener::E_LEFT, LOWORD(lParam), HIWORD(lParam));
		}
		break;
	case WM_RBUTTONDOWN:
		if(d_pMouseTarget) {
			d_pMouseTarget->onButtonDown(I_MouseListener::E_RIGHT, LOWORD(lParam), HIWORD(lParam));
		}
		break;
	case WM_RBUTTONUP:
		if(d_pMouseTarget) {
			d_pMouseTarget->onButtonUp(I_MouseListener::E_RIGHT, LOWORD(lParam), HIWORD(lParam));
		}
		break;
	case WM_MBUTTONDOWN:
		if(d_pMouseTarget) {
			d_pMouseTarget->onButtonDown(I_MouseListener::E_MIDDLE, LOWORD(lParam), HIWORD(lParam));
		}
		break;
	case WM_MBUTTONUP:
		if(d_pMouseTarget) {
			d_pMouseTarget->onButtonUp(I_MouseListener::E_MIDDLE, LOWORD(lParam), HIWORD(lParam));
		}
		break;
	case WM_XBUTTONDOWN:
		if(d_pMouseTarget) { 
			if(HIWORD(wParam) == XBUTTON1)
				d_pMouseTarget->onButtonDown(I_MouseListener::E_SIDE1, LOWORD(lParam), HIWORD(lParam));
			else
				d_pMouseTarget->onButtonDown(I_MouseListener::E_SIDE2, LOWORD(lParam), HIWORD(lParam));
		}
		break;
	case WM_XBUTTONUP:
		if(d_pMouseTarget) {
