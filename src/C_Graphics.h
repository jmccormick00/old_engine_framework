...
private:
	...
	ID3D10DepthStencilState* d_depthDisabledStencilState;
	ID3D10DepthStencilState* d_depthStencilState;
...
public:
	...
	bool init(const C_Window& window, bool fullScreen);
	...
	void enable3DMode();
	void enable2DMode();
	void setDefaultBlendStateAfterFontDraw();

	void presentFrame() { d_SwapChain->Present(0, 0); }

	void drawText(const std::string& text, const D3DXCOLOR& color, unsigned topLeftX, unsigned topLeftY, unsigned bottomRightX, unsigned bottomRightY);
};
